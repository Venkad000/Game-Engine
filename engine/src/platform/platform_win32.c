...
b8 platform_startup (
    ...
    if (!RegisterClassA(&wc)) {
        MessageBoxA(0, "Windown Registration failed", "Error", MB_ICONEXCLAMATION | MB_OK);
        return FALSE;
    }
    ...
    HWND handle = CreateWindowExA(
        window_ex_style, "game_window_class", application_name,
...
void platfomr_free(void* block, b8 aligned) {
    free(block);
}
